// we'll sort in desc order because we want smaller element on right side
// condition we need to satisfy is
//  arr[i] > arr[j]
//  index of arr[i] < index of arr[j] (i < j) count[i] += right - j + 1 coz in desc order right - j + 1 will give us count of smaller elements on right
// i++
// if arr[i] < arr[j] then we need to move j++
class Solution
{
public:
